...
#ifdef _WIN32
const std::string DAEMON_FILENAME = "kontorcoind.exe";
#else
const std::string DAEMON_FILENAME = "kontorcoind";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_kontorcoind_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"kontorcoind" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), "kontorcoind", "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
