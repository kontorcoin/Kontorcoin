...
#ifdef _WIN32
const std::string kontorcoinDaemon = "kontorcoind.exe";
#else
const std::string kontorcoinDaemon = "kontorcoind";
#endif
...
TestNodeConfiguration TestNetworkBuilder::buildNodeConfiguration(size_t index) {
  ...
  cfg.daemonPath = kontorcoinDaemon; // default
  cfg.testnet = testnet;
  cfg.logFile = "test_kontorcoind" + std::to_string(index) + ".log";
